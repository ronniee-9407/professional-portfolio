                ...
                <div className="grid md:grid-cols-2 gap-12 items-center">
                    ...
                    <motion.div variants={itemVariants} className="relative">
                        <div className="relative w-full max-w-md mx-auto">
                            ...
                            >
                                <div className="aspect-square bg-gradient-to-br from-primary-500/10 to-accent-500/10 rounded-2xl flex items-center justify-center">
                                    <span className="text-9xl">👨‍💻</span>
                                </div>
                            </motion.div>

                            {/* Decorative elements - reduced animation scale for mobile performance */}
                            <motion.div
                                className="absolute -top-4 -right-4 w-24 h-24 bg-primary-500/10 rounded-full blur-xl hidden sm:block"
                                animate={{ scale: [1, 1.1, 1] }}
                                transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
                            />
                            <motion.div
                                className="absolute -bottom-4 -left-4 w-32 h-32 bg-accent-500/10 rounded-full blur-xl hidden sm:block"
                    ...
                    <motion.div variants={itemVariants} className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3 pt-4">
                            {['React', 'TypeScript', 'Node.js', 'MongoDB', 'TailwindCSS', 'AWS'].map((tech) => (
                                <motion.span
                                    key={tech}
                                    className="px-4 py-2 glass-card rounded-full text-sm font-medium text-primary-600 dark:text-primary-300 border border-primary-500/20"
                ...
                >
                    {stats.map((stat) => (
                        <motion.div
                            ...
                            className="glass-card p-6 rounded-2xl text-center hover:bg-black/5 dark:hover:bg-white/10 transition-colors group"
                            whileHover={{ scale: 1.05, y: -5 }}
                        >
                            <div className="text-4xl font-bold gradient-text mb-2 group-hover:scale-110 transition-transform">
                                {stat.value}
                            </div>
                            <div className="text-slate-600 dark:text-gray-400 text-sm font-medium">{stat.label}</div>
                        </motion.div>
                    ))}
                </motion.div>
            </motion.div>
